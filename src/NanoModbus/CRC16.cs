namespace Nano.Modbus {
    /// <summary>
    ///     Modbus 工具类，计算CRC 16校验
    /// </summary>
    internal static class CRC16 {
        private static readonly byte[] AuchCrcHi = {
            
        };

        private static readonly byte[] AuchCrcLo = {
            
        };

        /// <summary>
        ///     计算 CRC
        ///     @return Crc16 双字节数组 高位在前，地低在后
        /// </summary>
        public static byte[] Calculate(byte[] src, int len) {
            return Calculate(src, 0, len);
        }

        /// <summary>
        ///     计算 CRC
        ///     @return Crc16 双字节数组 高位在前，地低在后
        /// </summary>
        public static byte[] Calculate(byte[] src, int off, int len) {
            if (len >= src.Length) {
                len = src.Length;
            }
            byte uchCrcHi = 0xff;
            byte uchCrcLo = 0xff;
            for (var i = off; i < len; i++) {
                var index = (uchCrcHi ^ src[i]) & 0xff;
                uchCrcHi = (byte) (uchCrcLo ^ AuchCrcHi[index]);
                uchCrcLo = AuchCrcLo[index];
            }
            return new[] {uchCrcHi, uchCrcLo};
        }

        /// <summary>
        ///     计算 CRC
        ///     @return Crc16 双字节数组 高位在前，地低在后
        /// </summary>
        public static byte[] Calculate(byte[] src1, byte[] src2, int off, int len) {
            int index;
            byte uchCrcHi = 0xff;
            byte uchCrcLo = 0xff;
            for (var i = 0; i < src1.Length; i++) {
                index = (uchCrcHi ^ src1[i]) & 0xff;
                uchCrcHi = (byte) (uchCrcLo ^ AuchCrcHi[index]);
                uchCrcLo = AuchCrcLo[index];
            }
            for (var i = off; i < len; i++) {
                index = (uchCrcHi ^ src2[i]) & 0xff;
                uchCrcHi = (byte) (uchCrcLo ^ AuchCrcHi[index]);
                uchCrcLo = AuchCrcLo[index];
            }
            return new[] {uchCrcHi, uchCrcLo};
        }

        /// <summary>
        ///     计算 CRC
        ///     @return Crc16 双字节数组 高位在前，地低在后
        /// </summary>
        public static byte[] Calculate(byte[] src1, byte[] src2) {
            return Calculate(src1, src2, 0, src2.Length);
        }

        public static byte[] Calculate(byte[] src) {
            return Calculate(src, 0, src.Length);
        }
    }
}